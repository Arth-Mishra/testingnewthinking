...
const Chatbot = ({ isChatOpen, setIsChatOpen, micOpen, setMicOpen }) => {
  ...
  const elevenLabs = useCallback(
    (text) => {
      ...
      fetch(
        "https://api.elevenlabs.io/v1/text-to-speech/mpfMerUvLuG5jvSICjfV/stream?optimize_streaming_latency=2",
        {
          method: "POST",
          headers: {
      ...
      console.log("First_TIme Stream TIme: " + new Date().toLocaleTimeString());
          ...
          // device?playAudioStream(stream):playStreamingAudio(stream);
  ...
  const playStreamingAudio = async (stream) => {
    logContent("in PlayStreamingAudio");
  ...
  const handleMicButtonClick = () => {
    ...
    //Code for starting the audioContext in in ios
